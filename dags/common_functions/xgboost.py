    ...
    while not yesterday or not day_before_yesterday:  # 전날을 찾을 때까지 반복문 실행
        ...
        if subtract_date.weekday() < 5:  # i일 전의 날이 주중이면 실행
            if not yesterday:
                ...
                day_before_yesterday = subtract_date
                break
        i += 1  # 전날과 전전날을 아직 못 찾았으므로 i + 1

    #trained_day: 이미 훈련이 끝마친 모델 날짜 / training_day: 뉴스 점수를 계속 업데이트하여 계속 훈련할 날짜
    trained_day, training_day = None, None

    if today.hour <16: #주식 시장 종료 이전이면 -> 예측할 종가 날짜: today, 뉴스 점수 업뎃 날짜: yesterday, 구현된 최종 모델: day_before_yesterday
        trained_day = day_before_yesterday
        training_day = yesterday
    else: #주식 시장 종료 이후면 -> 예측할 종가 날짜: tomorrow, 뉴스 점수 업뎃 날짜: today, 구현된 최종 모델: yesterday
        trained_day = yesterday
        training_day = today
